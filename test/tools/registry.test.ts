...
import type { DiscordAdapter } from "../../src/discordAdapter.js";
import { DiscordRequestError } from "../../src/discordAdapter.js";
...
describe("buildToolHandler", () => {
  it("returns a success envelope for a 2xx Discord response", async () => {
    const adapter: DiscordAdapter = { request: vi.fn().mockResolvedValue({ status: 200, body: { id: "1" }, headers: {} }) };
  ...
  it("maps a timeout DiscordRequestError to a local DISCORD_REQUEST_TIMEOUT error envelope", async () => {
    const adapter: DiscordAdapter = {
      request: vi.fn().mockRejectedValue(new DiscordRequestError("Discord request failed: timeout", "timeout")),
    };
    const handler = buildToolHandler(adapter, def, createLogger("error"));
    const result = await handler({});
  ...
  it("short-circuits with INVALID_ARGUMENTS and never calls Discord when def.validate() fails", async () => {
    ...
    const defWithValidate: ToolDef = { ...def, validate: () => "at most one of around, before, after may be set" };
    const handler = buildToolHandler(adapter, defWithValidate, createLogger("error"));
