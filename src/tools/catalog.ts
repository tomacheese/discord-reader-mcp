...
/** Picks the given keys (when defined) from `args` into a Discord query-parameter object. */
const qs = (
...
export const toolCatalog: ToolDef[] = [
  ...
  },
  {
    ...
    path: (a) => `/users/${String(a.user_id)}`,
  },
  {
    name: 'get_guilds',
    description:
      'List guilds the current user is in. Discord: Get Current User Guilds.',
    inputShape: {
  ...
  },
  {
    ...
    path: (a) => `/guilds/${String(a.guild_id)}`,
  ...
  },
  {
    ...
    path: (a) => `/guilds/${String(a.guild_id)}/members/${String(a.user_id)}`,
  },
  {
    name: 'list_guild_members',
    description:
      'List guild members (requires GUILD_MEMBERS intent). Discord: List Guild Members.',
    inputShape: {
      guild_id: snowflake,
      limit: z.number().int().min(1).max(1000).optional(),
      after: snowflake.optional(),
    },
    path: (a) => `/guilds/${String(a.guild_id)}/members`,
    query: (a) => qs(a, ['limit', 'after']),
  },
  {
    name: 'search_guild_members',
    description:
      'Search guild members by name/nickname prefix. Discord: Search Guild Members.',
    inputShape: {
      guild_id: snowflake,
      query: z.string().min(1),
      limit: z.number().int().min(1).max(1000).optional(),
    },
    path: (a) => `/guilds/${String(a.guild_id)}/members/search`,
  ...
  },
  {
    ...
    inputShape: { guild_id: snowflake },
    path: (a) => `/guilds/${String(a.guild_id)}/roles`,
  },
  {
    name: 'get_guild_role_member_counts',
    description:
      'Get member counts per role. Discord: Get Guild Role Member Counts.',
    inputShape: { guild_id: snowflake },
    path: (a) => `/guilds/${String(a.guild_id)}/roles/member-counts`,
  },
  {
    ...
    validate: (a) => {
      ...
      return set.length > 1
        ? `at most one of around, before, after may be set (got: ${set.join(', ')})`
        : undefined
    },
    path: (a) => `/channels/${String(a.channel_id)}/messages`,
  ...
  },
  {
    ...
    path: (a) => `/guilds/${String(a.guild_id)}/messages/search`,
    query: (a) =>
      qs(a, [
        'limit',
        'offset',
  ...
  },
  {
    ...
    path: (a) =>
      `/channels/${String(a.channel_id)}/messages/${String(a.message_id)}`,
  },
  {
    name: 'get_reactions',
    description:
      'List users who reacted with an emoji. Discord: Get Reactions.',
    inputShape: {
      channel_id: snowflake,
    ...
    path: (a) =>
      `/channels/${String(a.channel_id)}/messages/${String(a.message_id)}/reactions/${encodeURIComponent(String(a.emoji))}`,
  ...
  },
  {
    ...
    path: (a) =>
      `/channels/${String(a.channel_id)}/polls/${String(a.message_id)}/answers/${String(a.answer_id)}`,
    query: (a) => qs(a, ['after', 'limit']),
  },
  {
    name: 'list_active_guild_threads',
    description:
      'List active threads in a guild. Discord: List Active Guild Threads.',
    inputShape: { guild_id: snowflake },
    path: (a) => `/guilds/${String(a.guild_id)}/threads/active`,
  },
  {
  ...
  },
  {
    ...
    path: (a) => `/channels/${String(a.channel_id)}/thread-members`,
    query: (a) => qs(a, ['with_member', 'after', 'limit']),
  },
  {
    name: 'get_guild_audit_log',
    description:
      "Get a guild's audit log (requires VIEW_AUDIT_LOG). Discord: Get Guild Audit Log.",
    inputShape: {
      guild_id: snowflake,
    ...
    path: (a) => `/guilds/${String(a.guild_id)}/audit-logs`,
    query: (a) => qs(a, ['user_id', 'action_type', 'before', 'after', 'limit']),
  },
  {
    name: 'get_guild_bans',
    description:
      'List guild bans (requires BAN_MEMBERS). Discord: Get Guild Bans.',
    inputShape: {
      guild_id: snowflake,
      before: snowflake.optional(),
      after: snowflake.optional(),
    ...
    path: (a) => `/guilds/${String(a.guild_id)}/bans`,
    query: (a) => qs(a, ['before', 'after', 'limit']),
  },
  {
    name: 'get_guild_invites',
    description:
      "List a guild's invites (requires MANAGE_GUILD or VIEW_AUDIT_LOG). Discord: Get Guild Invites.",
    inputShape: { guild_id: snowflake },
    path: (a) => `/guilds/${String(a.guild_id)}/invites`,
  },
  {
    name: 'list_scheduled_events',
    description:
      "List a guild's scheduled events. Discord: List Scheduled Events for Guild.",
    inputShape: {
      guild_id: snowflake,
